...
import com.samoyer.mianshixiong.constant.CommonConstant;
import com.samoyer.mianshixiong.exception.ThrowUtils;
import com.samoyer.mianshixiong.mapper.QuestionMapper;
...
public class QuestionServiceImpl extends ServiceImpl<QuestionMapper, Question> implements QuestionService {
     ...
     */
    @Override
    public QuestionVO getQuestionVO(Long loginUserId, Question question, HttpServletRequest request) {
        // 对象转封装类
        ...
        // 2.关联查询题目的点赞收藏量
        LambdaQueryWrapper<QuestionLikeCollect> wrapper = Wrappers.lambdaQuery(QuestionLikeCollect.class)
                .eq(QuestionLikeCollect::getQuestionId, question.getId());
        QuestionLikeCollect questionLikeCollect = questionLikeCollectService.getOne(wrapper);
        questionVO.setLikeCount(questionLikeCollect.getLikeCount());
        questionVO.setFavourCount(questionLikeCollect.getFavourCount());

        // 3.关联查询当前用户是否已经点赞或者收藏该题目
        LambdaQueryWrapper<QuestionIsLikeCollect> lambdaQueryWrapper = Wrappers.lambdaQuery(QuestionIsLikeCollect.class)
                .eq(QuestionIsLikeCollect::getUserId, loginUserId)
                .eq(QuestionIsLikeCollect::getQuestionId, question.getId());
        QuestionIsLikeCollect questionIsLikeCollect = questionIsLikeCollectService.getOne(lambdaQueryWrapper);
        //查询的时候如果发现表中没有当前用户对于该题目的点赞收藏信息，则插入一条
        Integer isLike = 0, isFavour = 0;
        if (questionIsLikeCollect == null) {
            //往点赞收藏表中插入数据
            questionIsLikeCollect = new QuestionIsLikeCollect();
